...
char* Item::itemDescription() const {
...
std::ifstream& Item::load(std::ifstream& ifstr) {
    ...
    ut.alocpy(m_itemDescription, temp);
    ...
    if (!operator bool()) {
        ...
        if (m_isLinear) {
            ostr << std::setw(5) << m_sku << " | "
                ...
                << std::right << std::setw(7) << std::setprecision(2) << std::fixed << std::setprecision(2) << m_itemPrice << " |";
        }
        else {
...
int Item::readSku(std::istream& istr) {
    ...
    int newSku;
    std::cout << "SKU: ";
    newSku = ut.getint(40000, 99999, nullptr, "Invalid Integer");
    return newSku;
}

char* Item::readDesc(std::istream& istr) {
    char temDesc[2000];
    temDesc[0] = '\0';
    ...
    istr.get(temDesc, 2000, '\n');
    if (istr.fail()) {
        m_state = "Console entry failed!";
        return nullptr;
    }

    ut.alocpy(m_itemDescription, temDesc);
    return m_itemDescription;
}
...
std::istream& Item::read(std::istream& istr) {
    ...
    // Read Description
    m_itemDescription = readDesc(istr);
