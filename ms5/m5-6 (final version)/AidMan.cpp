...
namespace sdds {
    ...
    void AidMan::updateQuantity() {
        ...
        char sub_desc[2000];
        std::cin.ignore();
        std::cin.getline(sub_desc, 2000);

        int rowNum = list(sub_desc);
        ...
        int index = search(sku);
        if (index == -1) {
            std::cout << "SKU not found!" << std::endl << std::endl;
            return;
        }
        ...
        case 0:
            ...
            break;
        case 1: {
            if (m_product[index]->qty() >= m_product[index]->qtyNeeded()) {
                std::cout << "Quantity Needed already fulfilled!" << std::endl << std::endl;
                break;
            }
            std::cout << "Quantity to add: ";
            int addQty = ut.getint(1, m_product[index]->qtyNeeded(), nullptr, "Invalid quantity!");

            m_product[index]->operator+=(addQty);
            std::cout << addQty << " items added!" << std::endl << std::endl;
            break;
        }
        case 2: {
            if (m_product[index]->qty() == 0) {
                std::cout << "Quantity on hand is zero!" << std::endl << std::endl;
                break;
            }
            std::cout << "Quantity to reduce: ";
            int reduceQty = ut.getint(1, m_product[index]->qtyNeeded(), nullptr, "Invalid quantity!");
            
            m_product[index]->operator-=(reduceQty);
            std::cout << reduceQty << " items removed!" << std::endl << std::endl;
            break;
        }
        default:
    ...
    void AidMan::sortItems() {
        ...
        do {
            swapped = false;
            for (int i = 0; i < m_numOfProducts - 1; ++i) {
                int qtyDiff1 = m_product[i]->qtyNeeded() - m_product[i]->qty();
                int qtyDiff2 = m_product[i + 1]->qtyNeeded() - m_product[i + 1]->qty();
                if (qtyDiff1 < qtyDiff2) {
                    std::swap(m_product[i], m_product[i + 1]);
                    swapped = true;
                }
            }
        } while (swapped);
    ...
    void AidMan::shipItems() {
        int shipped = 0;
        std::ofstream ofstr("shippingOrder.txt", std::ios::out);

        if (!ofstr.is_open()) {
            std::cerr << "Failed to open shippingOrder.txt for writing!" << std::endl;
            return;
        }
        ...
        for (int i = 0; i < m_numOfProducts; ++i) {
            if (m_product[i]->qtyNeeded() == m_product[i]->qty()) {
                
                ofstr << std::setw(4) <<std::setfill(' ') << shipped + 1 << " | ";
                m_product[i]->linear(true);
                m_product[i]->display(ofstr) << std::endl;
